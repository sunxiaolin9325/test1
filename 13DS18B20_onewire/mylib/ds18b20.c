...
void	Ds18b20_Init(osPriority Priority)
{
	...
	//osThreadDef(Task_Ds18b20, (osPriority)3, 0, 128);
	...
	//for(;;)
	{
		...
		if(Ds18b20Timeout>0)
		{
			for (i = 0; i < TempSensorCount; i++)
			{
				...
				f[i] = ds18b20[i].Temperature;
				a[i] = ds18b20[i].DataIsValid;
			}
		}
		else
		{
		...
		//delay_ms(_DS18B20_UPDATE_INTERVAL_MS);
		delay_ms(1000);
	}
}
