...
void ONEWIRE_LOW(OneWire_t *gp)
{
	...
	// GPIOx_BSRR(复位/置位寄存器)的高16位，写1：对相应的GPIOx_ODR复位
	gp->GPIOx->BSRRH = gp->GPIO_Pin;
}	
...
void ONEWIRE_HIGH(OneWire_t *gp)
{
	...
	// GPIOx_BSRR(复位/置位寄存器)的低16位，写1：对相应的GPIOx_ODR置位
	gp->GPIOx->BSRRL = gp->GPIO_Pin;
}	
...
void ONEWIRE_INPUT(OneWire_t *gp)
{
	...
	*/
	...
	initValue.GPIO_Pin = gp->GPIO_Pin;
	GPIO_Init(gp->GPIOx, &initValue);
}
